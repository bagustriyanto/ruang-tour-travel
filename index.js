...
const whitelistDomain = require('./config/config.json').whitelistDomain;
...
app.use(
  cors({
    ...
    origin: function (option, callback) {
      ...
      if (whitelistDomain.indexOf(option) === -1)
        return callback(
          new Error("CORS Policy error, your domain doesn't have access"),
          false
        );
